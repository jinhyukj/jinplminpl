...
# Tab3(캔버스/노트)
Tab3에는 PaintView라는 class를 구현하여 그림을 그릴 수 있게 하였으며, seekbar와 버튼을 추가하여 선의 굵기와 색을 조절할 수 있게 하였다. 또한 erase 버튼을 추가하여 그린 그림을 모두 삭제하는 기능을 추가하였으며 redo와 undo를 구현하는 버튼을 추가하였다. 마지막으로 그림을 그리다 화면이 계속 넘어가는 불편함을 고치기 위해 lock을 만들어 켜놓으면 화면이 넘어가지 못하도록 만들었다.
